#!/usr/bin/env python3
## COMPREHENSIVE TEST - 100 Cases Across All Tasks
...
# Generate summary report (robust parsing of model outputs)
print("\n" + "="*80)
...
def parse_esi(value):
    """Try to extract an ESI level (1-5) from a model output string.
    Returns int 1-5 or None if no valid level found.
    """
    try:
        if pd.isna(value):
            return None
        ...
        pass
    s = str(value).strip()
    # Quick direct numeric conversion
    if s.isdigit():
        n = int(s)
        return n if 1 <= n <= 5 else None
    # Search for standalone digit 1-5
    m = re.search(r"\b([1-5])\b", s)
    if m:
        return int(m.group(1))
    # Search patterns like 'ESI 2' or '<ESI 2>'
    m = re.search(r"ESI[^0-9]*([1-5])", s, flags=re.IGNORECASE)
    if m:
        return int(m.group(1))
    return None

# Create parsed integer columns for predictions and ground truth (if present)
df_test[f'triage_{MODEL_NAME}_general_int'] = df_test[f'triage_{MODEL_NAME}_general'].apply(parse_esi)
df_test[f'triage_{MODEL_NAME}_clinical_int'] = df_test[f'triage_{MODEL_NAME}_clinical'].apply(parse_esi)

if 'triage' in df_test.columns:
    # parse ground-truth triage into integer where possible
    df_test['triage_int'] = df_test['triage'].apply(parse_esi)

# Triage accuracy (General User)
if 'triage_int' in df_test.columns:
    valid_triage = df_test[df_test['triage_int'].notna()]
    if len(valid_triage) > 0:
        correct = (valid_triage['triage_int'] == valid_triage[f'triage_{MODEL_NAME}_general_int']).sum()
        within_1 = ((valid_triage['triage_int'] - valid_triage[f'triage_{MODEL_NAME}_general_int']).abs() <= 1).sum()
...
if 'triage_int' in df_test.columns:
    valid_triage = df_test[df_test['triage_int'].notna()]
    if len(valid_triage) > 0:
        correct = (valid_triage['triage_int'] == valid_triage[f'triage_{MODEL_NAME}_clinical_int']).sum()
        within_1 = ((valid_triage['triage_int'] - valid_triage[f'triage_{MODEL_NAME}_clinical_int']).abs() <= 1).sum()

        print(f"\n📊 TRIAGE (CLINICAL USER) - {len(valid_triage)} cases:")
        print(f"   Exact Match: {correct}/{len(valid_triage)} = {100*correct/len(valid_triage):.1f}%")
        print(f"   Within 1 Level: {within_1}/{len(valid_triage)} = {100*within_1/len(valid_triage):.1f}%")

# Prediction distribution (show parsed ints and fallback to raw strings count)
print(f"\n📈 TRIAGE DISTRIBUTION:")
print(f"   General User (parsed int counts):")
print(df_test[f'triage_{MODEL_NAME}_general_int'].value_counts(dropna=False).to_string())
print(f"\n   Clinical User (parsed int counts):")
print(df_test[f'triage_{MODEL_NAME}_clinical_int'].value_counts(dropna=False).to_string())
